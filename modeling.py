...
# Models exclusively for CHALLENGE TASK
...
class TransformerEncoder(nn.Module):
    def __init__(
        ...
        self.cnn = CustomCNN(cnn_layers, conv_channels, fc_dim)
        self.pos_encoder = PositionalEncoding(fc_dim)
        encoder_layers = nn.TransformerEncoderLayer(d_model=fc_dim, nhead=nhead)
    ...
    def forward(self, inputs, lengths):
        ...
        x = x.permute(1, 0, 2)  # Transformer expects (S, N, E) shape
        output = self.transformer_encoder(x)
        output = output.permute(1, 0, 2)  # Convert back to (N, S, E) shape
        return output


class TransformerDecoder(nn.Module):
    def __init__(self, n_vocab=28, hidden_dim=64, num_layers=2, nhead=8, dropout=0.5):
    ...
    def forward(self, input_seq, memory):
        ...
        embedded = embedded.permute(1, 0, 2)  # Transformer expects (S, N, E) shape
